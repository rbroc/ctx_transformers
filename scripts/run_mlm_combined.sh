...
python3 train_mlm_combined.py --log-path standard_innermask_slowupdate --dataset-name 10context_large --per-replica-batch-size 1 --dataset-size 100 --n-epochs 1 --start-epoch 0 --update-every 1 --mlm-type standard --n-layers 2 --aggregate add --reset-head

python3 train_mlm_combined.py --log-path biencoder_innermask_slowupdate --dataset-name 10context_large --per-replica-batch-size 1 --dataset-size 100 --n-epochs 1 --start-epoch 0 --update-every 1 --mlm-type biencoder --n-layers 3 --n-layers-context-encoder 1 --aggregate add --reset-head
